...
m2d <- lmer(as.numeric(rating1) ~ condition * classNExpr * region_Morava + (1|participant) + (1|item), data=items_with_probability_bott)
summary(m2d)

```

- better model
- help for the logistic regression: https://stats.oarc.ucla.edu/r/dae/ordinal-logistic-regression/
- and for ordinal logistic regression:
https://marissabarlaz.github.io/portfolio/ols/
- but at the end I use clmm since it is still ordered, not logistic (TRUE/FALSE is just the dependent variable)

```{r}
library(ordinal)
#library(MASS)

m2d_ord <- clmm(as.factor(rating1) ~ condition * classNExpr * region_Morava + + (1|participant) + (1|item), data=items_with_probability_bott)
summary(m2d_ord)
...
m2da <- lmer(as.numeric(rating1) ~ condition * classNExpr * age_under_27 + (1|participant) + (1|item), data=items_with_probability_bott)
summary(m2da)
...
items_with_probability_bott.tmp <- items_with_probability_bott %>%
    group_by(participant)

items_with_probability_bott.tmp$MA = mean(items_with_probability_bott.tmp$demographic1, na.rm=TRUE)

items_with_probability_bott.tmp$SD = sd(items_with_probability_bott.tmp$demographic1, na.rm=TRUE)

items_with_probability_bott.tmp <- items_with_probability_bott.tmp %>%
    mutate(AgeTransformed = (demographic1 - MA)/SD)

hist(items_with_probability_bott.tmp$AgeTransformed)

m2da_2 <- lmer(as.numeric(rating1) ~ condition * classNExpr * AgeTransformed + (1|participant) + (1|item), data=items_with_probability_bott.tmp)
summary(m2da_2)
...
m2dr <- lmer(as.numeric(rating1) ~ condition * classNExpr * reading_time_over_60_minutes + (1|participant) + (1|item), data=items_with_probability_bott)
summary(m2dr)
